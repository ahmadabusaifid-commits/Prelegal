// The Standard Terms below are a direct copy of `templates/Mutual-NDA.md`
// (Common Paper Mutual NDA v1.0). Keep in sync with that file until a
// build-time generator replaces this hand-maintained copy.
...
export function buildTemplateVars(data: NDAData): Record<string, string> {
  return {
    governingLaw: data.governingLaw || "[Governing Law]",
    jurisdiction: data.jurisdiction || "[Jurisdiction]",
  };
}

// Format an ISO date string (YYYY-MM-DD) as e.g. "May 17, 2026".
// Parses date components manually to avoid the UTC-midnight timezone shift
